...
import { algoNodeSettings, readState, writeState } from "./utils.mjs";
...
async function loadAllProperties() {
  ...
  return new Promise((resolve, reject) =>
    fs
      .createReadStream(PROPERTIES_FILE)
      .pipe(csv())
      .on("data", (row) => {
...
async function updateNFTNote(assetID) {
  const { server, port, token } = algoNodeSettings();
...
async function mintAll() {
  ...
  for (var i = 0; i < fileNames.length; i++) {
    ...
    if (state[fileName]) {
      console.log(`Already minted: ${fileName}`);
      continue;
    }
